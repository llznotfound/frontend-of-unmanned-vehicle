...
import Angler from '../../components/main/angle'
import SaveSpace from '../../components/main/savespace'
import MissionTabler from '../../components/main/table'
import * as coordtransform from 'coordtransform'
import {PREFIX, WorkingState,ManageState,ThemeState,PlansState,ManasState,ChoseState,UpdaRoadState,RecordState} from '../../models/main/index'
import {Map,Marker,MapvLayer} from 'react-bmap'
import {
  simuDataAction,
  addBoundPointAction,
  calculateRouteAction,
  findDistanceAction,
  sendBackRoutesAction,
  updateAllPointAction,
  updateRecordAction,
  getGpsPositionAction,
  updateTranAction,
  updateKeys,
  getPositionAction,
  updateAlertStationAction,
  updateThemeAction,
  sendStartAction,
  addPointAction,
  sendRoutesAction,
  updateWorkingPointAction,
  requestRelayChangeAction,
  onBottleClickAction,
  changeRelayAction,
  blinkAction,
  updateWorkingPoint,
  changeVehicle_NoAction, getWorkingPointsAction, getFileListAction
...
import {Button , Icon , Slider , Switch , Menu , message , Popover} from 'antd'
import {changeTimeAction, clearTimeAction, changeAlertTimeAction, clearAlertTimeAction, sendReturnAction, toggleModeAction} from '../../models/main/changeTime'
import { addBoundPoint} from '../../models/main/workingstatus';
...
class Main extends React.Component<MainProps & MainDispatch & ManageProps & ThemeProps & PlansProps & ManasProps & ChoseProps & UpdaRoadProps & RecordProps &vehicle_No, any> {
  ...
  render(){
    ...
    const Button2_1 = style ({ //预留2_1按钮样式
      position:'absolute',
      top:'36px',
      left:'624px',
      width:'100px',
    ...
    const Button2_2 = style ({ //直走/停止按钮样式
      position:'absolute',
      top:'36px',
      left:'742px',
      width:'100px',
    ...
    const popstyle = (  //弹出添加路径点的框
      <div>
        经度:<input type = "text"  placeholder={this.props.lng.toString()}/><br/>
        纬度:<input type = "text"  placeholder={this.props.lat.toString()}/><br/>
    ...
    return (
      <div className = {wholeStyle} >
        ...
        <div className={vehicle_select}>
          <div className={vehicle_selectTextStyle}>当前选择车辆</div>
        ...
        <div className = {status} >
          ...
          <div className = {timeTextStyle} >预计剩余时间</div>
          <div className = {timeStyle} >{this.getTime()}</div>
          <div className = {distanceTextStyle} >已出发</div>
          <div className = {distanceStyle} >12
            <span className = {distanceCompanyStyle} > km </span>
          </div>
          <div className = {missionPointStyle} > 任务点剩余</div>
        ...
        {/*速度栏 最大速度3m/s*/}
        <div className = {speed}>
          <div className = {speedTextStyle} >速度</div>
          ...
          <div className={accelerate}>
            <div className={accTextStyle}>加速度</div>
         ...
         {/*瞬时功率栏 */}
         <div className={power}>
            <div className={powerTextStyle}>功率</div>
        ...
        <div className={location}>
            ...
            <div className={locationNumStyle}>N{this.props.lng.toFixed(6)}<div>E{this.props.lat.toFixed(6)}</div>
            </div>
        </div>

        {/*运动角度栏 */}
        <div className = {sport} >
          <div className = {sportTextStyle} >北向角度</div>
        ...
        <Button className = {Button1_1} type = "primary" onClick = {(() => this.sendAction(0))} >
         ...
         {/* 返航按钮 */}
        <Button className={Button1_2} type = "primary"onClick = {(() => this.sendAction(30))}>
          <div className={buttonIcon} ><Icon type="home" /></div>
        ...
        {/*</Button>*/}
        <Button className = {Button1_3} type = "primary" onClick = {(() => this.sendAction(10))}>
          <div className = {buttonIcon} ><Icon type='home' /></div>
          <div className = {buttonText} >停止</div>
        </Button>
        {/* 更新路径按钮 */}
        <Button className={Button1_4} type="primary"onClick = {(() => this.sendRoutes(50))}>
          <div className={buttonIcon} ><Icon type="home" /></div>
          <div className = {buttonText} > 更新路径</div>
        </Button>
        {/*预留2_1按钮
        <Button className = {Button2_1} type = "primary" disabled={Automode?true:false} >
          <div className = {buttonIcon} ><Icon type="home" /></div>
          <div className = {buttonText} > BUT2_1</div>
        </Button>

        直行/停止按钮
        <Button className = {Button2_2} type = "primary" onClick = {(() => this.sendAction(4))}>
          <div className = {buttonIcon} ><Icon type={pra == "停止"?'pause':'arrow-up'} /></div>
          <div className = {buttonText} >{pra1}</div>
        </Button>

        左转按钮
        <Button className = {Button2_3} type = "primary" onClick = {(() => this.sendAction(20))}>
          <div className = {buttonIcon} ><Icon type="arrow-left" /></div>
          <div className = {buttonText} >左转</div>
        </Button>

        右转按钮
        <Button className = {Button2_4} type = "primary" onClick = {(() => this.sendAction(15))}>
          <div className = {buttonIcon} ><Icon type="arrow-right" /></div>
          <div className = {buttonText} >右转</div>
        </Button>

        预留2_5按钮
        <Button className = {Button2_5} type = "primary" onClick = {(() => this.sendAction(10))}>
          <div className = {buttonIcon} ><Icon type="link" /></div>
          <div className = {buttonText} >停止</div>
        </Button>*/}
        ...
        <div className = {mapStyle} >
          ...
          <Popover content = {popstyle} title="调整经纬度">
            <Button className={mapButton} type="primary" onClick={(() => this.addPoint(this.props.lat.toFixed(8),this.props.lng.toFixed(8)))}>添加路径点</Button>
          </Popover>
          ...
          <Button className = {lineButton} type = "primary" onClick = {(() => this.focuPoint())}>密集路线</Button>
        ...
        <div className = {missionPoint} >
          ...
          <MissionTabler   workingPoint={this.props.workingPoint} lng={this.props.lng} lat={this.props.lat} theme={this.props.theme}/>*/}
          <Tabs defaultActiveKey="0" onChange={(index) => this.callback(index)}>
          {
            this.props.fileList.map((item, index) => {
              return <TabPane tab={"Tab " + item} key={index}>
  ...
  callback(index) {
    ...
    this.props.dispatch(getWorkingPointsAction({car: this.props.vehicle_No, file_name: this.props.fileList[index]}))
  }
  ...
  addPoint(lat,lng){
    ...
    point[0]=lat
    point[1]=lng
    this.props.dispatch(addPointAction(point))
    //message.success(point, 3)
  }
  //计算夹角
  sendRoutes(command:number){
     this.props.dispatch(sendRoutesAction(command))
  }
  chanslationPoint(){
    var resultPoints = []; //记录转换后的结果集
...
isPointOnPolyline(point, polyline){
  ...
  //摄像头登录
  checkIncamera(){
    var szIP = this.props.keysmap=="camera"?'192.168.1.107':'192.168.1.108'
    var szPort = this.props.keysmap=="camera"?'87':'86'
    var szUsername = 'admin'
    var szPassword = 'camera2018'
    var self = this
 ...
 */
  ...
  renderBattery(){  //根据电量渲染电池组件
    ...
    var batte = this.getBatteryInfor()
    if(isNaN(batte))batte = 28
    var colors;
