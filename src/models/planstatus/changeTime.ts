...
import {PREFIX, TimeState, PlanState} from './index'
import {IP} from '../main'
...
export function changeTime(state: TimeState) {
  let newState = {...state}
  if(newState.time == 0) newState.time = 9
  else newState.time = newState.time - 1
  return newState
}
...
export function changeAlertTime(state: PlanState) {
  let newState = {...state}
...
export function clearAlertTime(state: PlanState) {
  let newState = {...state}
...
export function toggleMode(state: PlanState) {
  let newState = {...state}
